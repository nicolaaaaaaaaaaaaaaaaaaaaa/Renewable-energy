...
function two_price_risk_analysis(Selected_scenarios, beta)
    ...
    @constraint(model_1,zeta_limit[w in 1:NSS],-sum(Selected_scenarios[w][2][t]*p_DA[t]+(0.9 + 0.1*(Selected_scenarios[w][3][t]))*Selected_scenarios[w][2][t]*Delta_up[t,w]-(1.2+0.2*(Selected_scenarios[w][3][t]-1))*Selected_scenarios[w][2][t]*Delta_down[t,w] for t in 1:T)
    +zeta-eta[w]<=0)
