...
#r_prod_limit_up =   [40 40  70  180 60  30  30  50   0  0    60  40 0 0 0 0 0 0 ]
#r_prod_limit_down = [40 40  70  180 60  30  30  50   0   0   60  40 0 0 0 0 0 0 ]
#r_price_up =  [15 15  24  25  28  16  16  0   0   0   14  16 0 0 0 0 0 0 ]
#r_price_down =[11 11  16  17  23  7   7   0   0   0   8   8  0 0 0 0 0 0 ]
...
if termination_status(model_1) == MOI.OPTIMAL
    ...
    @variable(model_2, q_electrolyzer_prod[1:P,1:T]>=0)


    """ Objective function """

    @objective(model_2, Max, sum(demand_utilities[i,t]*q_demand2[i,t] for i in 1:D, t in 1:T) - sum(prod_price[i]*(q_prod2[i,t] + q_electrolyzer_prod[i,t]) for i in 1:P, t in 1:T))

    """ Constraints """
    #Limit of the quantity of energy produced and consumed
    @constraint(model_2, Production_limit[p in 1:P, t in 1:T],  reserve_optimal_down[p,t] <= q_prod2[p,t] +q_electrolyzer_prod[p,t] <= prod_capacities[p,t] - reserve_optimal_up[p,t])
    ...
    @constraint(model_2, Ramp_limit[p in 1:P, t in 2:T], ramp_limit[p]>=(q_prod2[p,t]+q_electrolyzer_prod[p,t]-q_prod2[p,t-1]-q_electrolyzer_prod[p,t-1])>=-ramp_limit[p])

    #Electrolyzer constraint, the demand for hydrogen should be met by the end of the day by the concerned wind farm 
    @constraint(model_2, Demand_electrolyzer[p in 1:P], demand_electrolyzer[p]==sum(q_electrolyzer_prod[p,t] for t in 1:T)*18/1000)

    #Electrolyzer production Limit
    @constraint(model_2, Hydrogen_limit[p in 1:P, t in 1:T], hydrogen_limit>=q_electrolyzer_prod[p,t])
    ...
    electrolyzer_prod = zeros(Float64,(P,T))
    ...
    if termination_status(model_2) == MOI.OPTIMAL
        ...
        for t in 1:T
            ...
            electrolyzer_prod[:,t] = value.(q_electrolyzer_prod[:,t])
            ...
            println("electrolyzer : $(round.(electrolyzer_prod[:,t], digits=2))")
        end
